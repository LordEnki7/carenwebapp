...
export default function Record() {
  ...
  // Check URL parameters for emergency mode + auto-start
  useEffect(() => {
    ...
    if (urlParams.get('autostart') === 'video') {
      setIsEmergencyMode(true);
      setPriority('high');
      setTitle('Emergency Recording');
      setDescription('Traffic stop — recording in progress');
      setRecordingType('video');
      // Dispatch after a brief delay so all other effects are registered
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent('autoStartVideoRecording', {
          detail: { source: 'panic_button' }
        }));
      }, 800);
    }
  }, []);
