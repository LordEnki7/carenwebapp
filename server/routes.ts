...
- Do NOT use markdown formatting — respond in plain text only`,
  ...
  // that production is actually running the code we just pushed.
  const SERVER_START_TIME = new Date().toISOString();
  let BUILD_INFO: { commit: string; shortCommit: string; branch: string; buildTime: string } = {
    commit: "unknown", shortCommit: "unknown", branch: "unknown", buildTime: "unknown",
  };
  try {
    // build-info.json is generated by scripts/write-build-info.cjs at deploy time
    // and copied into the Docker image. It's the source of truth for "what's deployed".
    const fsMod = await import("fs");
    const pathMod = await import("path");
    const candidates = [
      pathMod.join(process.cwd(), "dist", "public", "build-info.json"),
      pathMod.join(process.cwd(), "client", "public", "build-info.json"),
    ];
    for (const p of candidates) {
      if (fsMod.existsSync(p)) {
        BUILD_INFO = JSON.parse(fsMod.readFileSync(p, "utf-8"));
        console.log(`[VERSION] Loaded build-info: commit=${BUILD_INFO.shortCommit} branch=${BUILD_INFO.branch}`);
        break;
      }
    }
  } catch (e) {
    console.warn("[VERSION] Could not load build-info.json:", e);
  }
