...
const CAREN_NAME = 'C.A.R.E.N.™ Legal Access Network';
...
function email1({ contactName, firmName, state }: DripEmailParams): { subject: string; html: string; text: string } {
  ...
  const subject = `Join the C.A.R.E.N. Legal Access Network — Attorneys in ${state} Needed`;
  const html = brandedLayout(subject, `
    ...
    <p>We are actively building the <strong>C.A.R.E.N. Legal Access Network (CLAN)</strong> — a curated directory of verified attorneys across the country — and I believe <strong>${firmName}</strong> would be an excellent fit.</p>
    <p><strong>What CLAN membership means for your firm:</strong></p>
    <ul>
      <li><strong>Verified directory listing</strong> visible to thousands of C.A.R.E.N. users in ${state}</li>
    ...
    <div class="highlight-box">
      <p>🛡️ C.A.R.E.N. serves users in all 50 states with 467+ legal protections. Our users are actively seeking legal help at the moment they need it most.</p>
    </div>
...
function email2({ contactName, firmName, state }: DripEmailParams): { subject: string; html: string; text: string } {
  const name = contactName || `${firmName} Team`;
  const subject = `Following up — Your spot in the C.A.R.E.N. network (${state})`;
  const html = brandedLayout(subject, `
    <h2>Hi ${name},</h2>
    <p>I wanted to follow up on my previous message about joining the <strong>C.A.R.E.N. Legal Access Network</strong>. I know inboxes get busy, so I'm keeping this brief.</p>
    ...
    <ul>
      ...
      <li>⚡ <strong>Warm, motivated leads.</strong> Clients contact attorneys through C.A.R.E.N. while an incident is happening or immediately after — they're ready to engage.</li>
...
function email3({ contactName, firmName, state }: DripEmailParams): { subject: string; html: string; text: string } {
  const name = contactName || `${firmName} Team`;
  const subject = `How C.A.R.E.N. connects clients to attorneys in ${state}`;
  const html = brandedLayout(subject, `
    <h2>Hi ${name},</h2>
    <p>I want to give you a clearer picture of how clients actually find attorneys through C.A.R.E.N. — because the matching system is what makes this different from a typical directory.</p>
    <p><strong>Here's what happens when a C.A.R.E.N. user needs legal help:</strong></p>
    <ul>
    ...
    <div class="highlight-box">
      <p>💼 Most C.A.R.E.N. users are seeking help with traffic-related matters, civil rights questions, or general criminal defense. Attorneys licensed in ${state} with those specialties consistently rank highest in matches.</p>
    </div>
  ...
  `);
  const text = `Hi ${name},\n\nHere's how C.A.R.E.N. client matching works: GPS-based location detection → case type description → AI matches attorneys by state, specialty, and availability.\n\nIf ${firmName} handles traffic defense, civil rights, or criminal defense in ${state}, you're a great fit.\n\nApply: ${APPLY_URL}\n\nShawn Williams`;
  return { subject, html, text };
}
...
function email4({ contactName, firmName, state }: DripEmailParams): { subject: string; html: string; text: string } {
  const name = contactName || `${firmName} Team`;
  const subject = `Limited verified spots left in ${state} — C.A.R.E.N. CLAN`;
  const html = brandedLayout(subject, `
    <h2>Hi ${name},</h2>
    <p>I've reached out a few times about the <strong>C.A.R.E.N. Legal Access Network</strong>, and I want to be transparent: we are approaching capacity for verified attorney slots in <strong>${state}</strong>.</p>
    ...
    <ul>
      <li>✅ "Verified by C.A.R.E.N." badge on your profile</li>
      ...
      <li>✅ Access to C.A.R.E.N.'s growing base of subscribers — users who are actively engaged with their legal rights</li>
    </ul>
...
function email5({ contactName, firmName, state }: DripEmailParams): { subject: string; html: string; text: string } {
  const name = contactName || `${firmName} Team`;
  const subject = `Closing your file — C.A.R.E.N. CLAN (${firmName})`;
  const html = brandedLayout(subject, `
    <h2>Hi ${name},</h2>
    <p>I don't want to keep filling your inbox, so this will be my last message about the C.A.R.E.N. Legal Access Network — for now.</p>
    ...
    <ul>
      <li>We are on track to expand C.A.R.E.N. to iOS, Android, and desktop — giving attorneys in the network exposure across all platforms</li>
