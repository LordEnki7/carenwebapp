    ...
    if (video) {
      ...
      if (fs.existsSync(srcPath)) {
        ...
        for (const target of TARGETS) {
          ...
          try {
            // -f lavfi -i anullsrc adds a silent AAC stereo track — required by Apple App Store.
            // Playwright screen recordings have no audio; App Store rejects videos without a valid audio stream.
            execSync(`ffmpeg -y -i "${srcPath}" -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -vf "scale=${target.width}:${target.height}:flags=lanczos" -c:v libx264 -pix_fmt yuv420p -preset medium -crf 20 -r 30 -t 30 -c:a aac -b:a 128k -map 0:v:0 -map 1:a:0 -shortest "${mp4Path}" 2>/dev/null`, { timeout: 30000 });
